...
function ListaTareas({ tipo, tareas, marcarTarea, reiniciar, tema, onIconLongPress }) {
  ...
  return (
    <ScrollView style={styles.lista} contentContainerStyle={styles.listaContent}>
      {tareas.map((task) => {
        const iconoImagen = (tipo === 'diaria' && task.id === 'd1') ? ICONO_DESAYUNO : (tipo === 'diaria' && task.id === 'd2') ? ICONO_TOMAR_DESAYUNO : (tipo === 'diaria' && task.id === 'd5') ? ICONO_TOMAR_ONCE : (tipo === 'semanal' && task.id === 's1') ? ICONO_LAVAR_ROPA : (tipo === 'semanal' && task.id === 's2') ? ICONO_PLANCHAR : (tipo === 'semanal' && task.id === 's3') ? ICONO_LIMPIAR_COCINA : (tipo === 'semanal' && task.id === 's4') ? ICONO_LIMPIAR_REFRIGERADOR : (tipo === 'semanal' && task.id === 's6') ? ICONO_ORDENAR_ARMARIOS : (tipo === 'diaria' && task.id === 'd9') ? ICONO_BARRER_TRAPEAR : (tipo === 'diaria' && task.id === 'd3') ? ICONO_BANO : (tipo === 'diaria' && task.id === 'd4') ? ICONO_TENDER_CAMA : (tipo === 'diaria' && task.id === 'd6') ? ICONO_SACAR_BASURA : (tipo === 'diaria' && task.id === 'd7') ? ICONO_LIMPIAR_POLVO : (tipo === 'diaria' && task.id === 'd8') ? ICONO_LAVAR_LOZA : (tipo === 'mensual' && task.id === 'm1') ? ICONO_LIMPIAR_VENTANAS : (tipo === 'mensual' && task.id === 'm2') ? ICONO_ASPIRAR : (tipo === 'mensual' && task.id === 'm3') ? ICONO_REVISAR_DESPENSA : (tipo === 'mensual' && task.id === 'm4') ? ICONO_LIMPIAR_LAMPARAS : (tipo === 'anual' && task.id === 'a1') ? ICONO_LIMPIEZA_PROFUNDA : (tipo === 'anual' && task.id === 'a3') ? ICONO_ORDENAR_DONAR : null;

...
function iconStyleForTask(tipo, taskId) {
  ...
  if (tipo === 'diaria' && taskId === 'd6') return styles.taskIconImagePolvo;
...
function ListaTareas({ tipo, tareas, marcarTarea, reiniciar, tema }) {
  ...
  return (
    <ScrollView style={styles.lista} contentContainerStyle={styles.listaContent}>
      {tareas.map((task) => {
        const iconoImagen = (tipo === 'semanal' && task.id === 's1') ? ICONO_LAVAR_ROPA : (tipo === 'semanal' && task.id === 's2') ? ICONO_PLANCHAR : (tipo === 'semanal' && task.id === 's5') ? ICONO_ORDENAR_ARMARIOS : (tipo === 'diaria' && task.id === 'd1') ? ICONO_BARRER_TRAPEAR : (tipo === 'diaria' && task.id === 'd3') ? ICONO_BANO : (tipo === 'diaria' && task.id === 'd4') ? ICONO_TENDER_CAMA : (tipo === 'diaria' && task.id === 'd6') ? ICONO_LIMPIAR_POLVO : (tipo === 'mensual' && task.id === 'm1') ? ICONO_LIMPIAR_VENTANAS : null;
        ...
        >
          ...
          <Text style={[styles.taskText, (tipo === 'diaria' && task.id === 'd3') && styles.taskTextBanoOffset, (tipo === 'diaria' && task.id === 'd4') && styles.taskTextTenderCama, (tipo === 'mensual' && task.id === 'm1') && styles.taskTextLimpiarVentanas, task.hecha && { color: t.taskTextDone, textDecorationLine: 'line-through' }]}>{tipo === 'diaria' && task.id === 'd6' ? `${task.nombre} ✨` : task.nombre}</Text>
